...
pub fn encrypt(cipher:&Aes256Gcm, nonce:&Nonce<consts::U12>, bytes:&[u8]) -> Result<Vec<u8>, aes_gcm::Error> {
	cipher.encrypt(&nonce, bytes)
}

pub fn decrypt(cipher:&Aes256Gcm, nonce:&Nonce<consts::U12>, encrypted_bytes:&[u8]) -> Result<Vec<u8>, aes_gcm::Error> {
	cipher.decrypt(&nonce, encrypted_bytes)
}
...
mod tests {
    ...
    fn test_encrypt() {
		...
		let nonce = nonce_96_from_bytes(nonce_bytes);
		...
		let expected = [56u8, 239, 201, 242, 104, 254, 92, 18, 186, 49, 165, 166, 60, 211, 118, 19, 92, 218, 63, 76, 101, 225, 16, 43, 15, 123, 95, 201, 251, 203, 194, 43, 89, 226, 9, 237, 183, 74, 160, 218, 169, 13, 104, 105, 238, 178, 18, 161, 15, 30, 24, 148, 21, 110, 12, 138, 57, 30, 100, 29, 108, 110, 206, 226, 110, 57, 57, 81, 80, 99, 205, 106, 52, 9, 41, 152, 185, 185, 171, 187];
    ...
    fn test_decrypt() {
		...
		let encrypted_bytes = [56u8, 239, 201, 242, 104, 254, 92, 18, 186, 49, 165, 166, 60, 211, 118, 19, 92, 218, 63, 76, 101, 225, 16, 43, 15, 123, 95, 201, 251, 203, 194, 43, 89, 226, 9, 237, 183, 74, 160, 218, 169, 13, 104, 105, 238, 178, 18, 161, 15, 30, 24, 148, 21, 110, 12, 138, 57, 30, 100, 29, 108, 110, 206, 226, 110, 57, 57, 81, 80, 99, 205, 106, 52, 9, 41, 152, 185, 185, 171, 187];

		let result = decrypt(&cipher, &nonce, &encrypted_bytes).unwrap();
    ...
    fn test_encrypt_and_decrypt() {
		...
		let decrypted = decrypt(&cipher, &nonce, &encrypted).unwrap();
